...
import de.repictures.wzz.internet.postComment;
...
public class PostComment extends DialogFragment {
    ...
    public PostComment(String witz_raw) {
        extra = witz_raw.split("~");
    }
                ...
                .setPositiveButton(R.string.add_joke, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        if (writeWizzle.getText().toString() != ""){
                            new Thread(new postComment(writeWizzle.getText().toString(), extra[0], liesKey())).start();
                        }
                    }
                })
